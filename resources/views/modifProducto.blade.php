@extends('plantilla')

@section('principal')



<ul style='color:red' class="errores">
    @foreach ($errors->all() as $error)
    <li>
        {{$error}}
    </li>
    @endforeach
</ul>

<div class="container" style="position: relative; left: 0px; top: 105px;">
     <div class="card">
    <div class=" card-body">
    <form class= "" action="/modifProducto" method="post" enctype="multipart/form-data">
{{csrf_field() }}

    <div class='form-group row'>     
    <label for="nombre">Nombre:</label>
    <input type="text" name="nombre" value="{{$producto->nombre}}">
    </div>
<div class="form-group row">
    <label for="">Descripcion:</label>
    <input type="text" name="descripcion" value="{{$producto->descripcion}}">
</div>
<div class="form-group row">
    <label for="precio">Precio:</label>
    <input type="text" name="precio" value="{{$producto->precio}}">
</div>
<div class="form-group row">
    <label for="precio">Stock:</label>
    <input type="text" name="stock" value="{{$producto->stock}}">
</div>
<div class="form-group row">
    <label for="nombre_marca">Marca:</label>
    <input type="text" name="nombre_marca" value="{{$producto->nombre_marca}}">
</div>
<div class="form-group row">
    <label for="nombre_categoria">Categoria:</label>
    <input type="text" name="nombre_categoria" value="{{$producto->nombre_categoria}}">
</div>

</div class='form-group row'>
<label for="">Foto 1: {{$producto->poster}}</label>
<input type="file" name="poster" id="">
<div>

</div class='form-group row'>
<label for="">Foto 2: {{$producto->poster}}</label>
<input type="file" name="poster1" id="">
<div>

    </div class='form-group row'>
<label for="">Foto 3: {{$producto->poster}}</label>
<input type="file" name="poster2" id="">
<div>

<div class="form-group row">
    <input type="submit" name="" value="Agregar producto">
    </div>
</form>

</div>
</div>
</div>
@endsection